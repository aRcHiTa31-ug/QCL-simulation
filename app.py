...
This simulator integrates multiple scientific modules into one platform for
design, simulation, optimization and validation of 3  layered 4.6um QCL.
...
if inputs["run_simulation"]:
    ...
    try:
        ...
        with st.spinner("Running Quantum Cascade Laser Simulation..."):
            ...
            # ==========================================================
            # Parameter Summary (2 columns, instead of one long
            # vertical list of st.write calls that wasted vertical
            # space)
            # ==========================================================
            ...
            with viz_tab_wavelength:
                ...
                with col4:
                    ...
                    st.pyplot(fig)
                    # Wavenumber = 1/wavelength decreases monotonically
                    # with wavelength, so its maximum over the sweep
                    # occurs at wavelength_min - same relationship
                    # calculate_peak_frequency relies on for frequency.
                    peak_wavenumber = PhysicsEngine.calculate_wavenumber(
                        wavelength=wavelength_min
                    )
                    st.metric("Peak Wavenumber", f"{peak_wavenumber:.4g} m⁻¹")
                with col5:
                    ...
                    st.pyplot(fig)
                    # Same reasoning: photon momentum = h/wavelength is
                    # largest at the shortest wavelength in the sweep.
                    peak_photon_momentum = PhysicsEngine.calculate_photon_momentum(
                        wavelength=wavelength_min
                    )
                    st.metric("Peak Photon Momentum", f"{peak_photon_momentum:.4g} kg·m/s")
            ...
            if os.path.exists(MODEL_PATH):
                try:
                    ...
                    st.success("✅ Pre-trained RL model loaded successfully.")
                    model_loaded = True
            ...
            with rl_tab1:
                ...
                if model_loaded:
                    st.success("🟢 Status : Pre-trained model is ready.")
                    st.info("Training is optional. You can directly evaluate the trained agent.")
                else:
                    st.warning("No trained model found. Please train the RL agent.")
                    ...
                    if st.button("📊 Evaluate RL Agent"):
                        with st.spinner("Evaluating Agent..."):
                            ...
                            st.session_state.training_results = {
                                ...
                                "best_reward": evaluation_results["maximum_reward"],
                                "final_epsilon": rl_agent.epsilon,
                                "rewards": evaluation_results["episode_rewards"],
                                "epsilon_history": [rl_agent.epsilon] * evaluation_results["episodes"],
                                "completed": True,
                                "error": None,
                            }
            ...
            with rl_tab2:
                ...
                if st.session_state.training_results is not None:
                    ...
                    if results.get("episodes", 0) == 0:
                        st.warning(
                            "Training produced no completed episodes"
                            + (f": {results['error']}" if results.get("error") else ".")
                        )
                    else:
                        if not results.get("completed", True):
                            st.warning(
                                f"Training stopped early after {results['episodes']} "
                                f"episode(s)"
                                + (f": {results['error']}" if results.get("error") else ".")
                            )
                        ...
                        fig, ax = plt.subplots(figsize=(8, 4))
                        ax.plot(results["rewards"], linewidth=1.5, alpha=0.6, label="Reward")
                        if len(results["rewards"]) >= 10:
                            window = 10
                            moving_avg = np.convolve(
                                results["rewards"], np.ones(window) / window, mode="valid"
                            )
                            ax.plot(
                                range(window - 1, len(results["rewards"])),
                                moving_avg,
                                linewidth=2,
                                color="darkorange",
                        ...
                        fig, ax = plt.subplots(figsize=(8, 4))
                        ax.plot(results["epsilon_history"], linewidth=2, color="seagreen")
                        ax.set_xlabel("Episode")
                    ...
                    st.success("✅ Pre-trained RL model loaded.")
                    st.info(
                        "Training history is unavailable because only the trained Q-table "
                        "was loaded. Click Evaluate or Retrain to generate new statistics."
                    )
                else:
            ...
            for file in json_files:
                ...
                # ======================================================
                # Read Erwin Reference Results
                # ======================================================

                erwin = data["erwin_results"]

                validation_rows.append({

                    "File": os.path.basename(file),

                    "Simulator λ (µm)": round(wavelength * 1e6, 3),

                    "Erwin λ (µm)": erwin["wavelength"],

                    "Simulator Temp (K)": round(device_temperature, 2),

                    "Erwin Temp (K)": erwin["temperature"],

                    "Simulator States": simulator_states,

                    "Erwin States": erwin["states"],

                    "Simulator Layer Width (nm)": simulator_layer_width,

                    "Erwin Layer Width (nm)": erwin["layer_width"]
